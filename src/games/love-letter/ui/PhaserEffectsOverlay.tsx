...
import type { EffectPayload } from "../scene/LoveLetterEffectsScene";
  ...
  function PhaserEffectsOverlay(_props, ref) {
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        const { LoveLetterEffectsScene } = await import(
          "../scene/LoveLetterEffectsScene"
        );
        if (cancelled || !wrapRef.current) return;

        const scene = new LoveLetterEffectsScene();
        scene.setOnReady(() => {
          if (cancelled) return;
        ...
        // Track wrapper size and resize the Phaser canvas to match.
