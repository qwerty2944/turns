...
import type { EffectPayload } from "../scene/YeouidoEffectsScene";
  ...
  function PhaserEffectsOverlay(_props, ref) {
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        const { YeouidoEffectsScene } = await import(
          "../scene/YeouidoEffectsScene"
        );
        if (cancelled || !wrapRef.current) return;

        const scene = new YeouidoEffectsScene();
        scene.setOnReady(() => {
          if (cancelled) return;
