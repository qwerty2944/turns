...
export class LoveLetterEffectsScene extends Phaser.Scene {
  ...
  private spawnCardSprite(at: Anchor, cardKey: string, scale = 0.4) {
    const tex = this.textures.exists(`card-${cardKey}`)
      ? `card-${cardKey}`
      : "card-back";
    const s = this.add.image(at.x, at.y, tex);
    s.setDisplaySize(70 * scale, 105 * scale);
    s.setScale(scale);
    return s;
  }
  ...
  private fxGuardThrow(actor: Anchor, target?: Anchor, guess?: number) {
    ...
    const sprite = this.spawnCardSprite(actor, "guard", 0.55);
    sprite.setAlpha(0);
    this.tweens.add({
      targets: sprite,
      alpha: 1,
  ...
  private fxBaronCollide(actor: Anchor, target?: Anchor) {
    ...
    const a = this.spawnCardSprite(actor, "baron", 0.5);
    const b = this.spawnCardSprite(target, "baron", 0.5);
    this.tweens.add({
      targets: a,
      x: mid.x - 20,
      y: mid.y,
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        ...
        bodyA.setVelocity(-80, -150);
        bodyB.setVelocity(80, -150);
  ...
  private fxPrinceVortex(at: Anchor) {
    ...
    swirl.lineStyle(3, 0x7a3fff, 0.85);
    for (let i = 0; i < 4; i++) {
      const r = 12 + i * 8;
    ...
    const eject = this.spawnCardSprite(at, "back", 0.45);
    this.tweens.add({
      targets: eject,
      y: at.y - 80,
      alpha: 0,
  ...
  private fxKingSwap(actor: Anchor, target?: Anchor) {
    if (!target) return;
    const a = this.spawnCardSprite(actor, "back", 0.45);
    const b = this.spawnCardSprite(target, "back", 0.45);
    this.tweens.add({
      targets: a,
      x: target.x,
      y: target.y,
  ...
  private fxPrincessFall(seat: Anchor) {
    const start = { x: seat.x, y: -80 };
    const sprite = this.spawnCardSprite(start, "princess", 0.6);
  ...
  private fxGuardHit(target: Anchor) {
    const flash = this.add.rectangle(target.x, target.y, 110, 150, 0xff4d4d, 0.55);
    this.tweens.add({
      targets: flash,
      alpha: 0,
      scale: 1.4,
  ...
  private fxGuardMiss(actor: Anchor) {
    const txt = this.add
      .text(actor.x, actor.y - 30, "❌", { fontSize: "28px" })
      .setOrigin(0.5);
    this.tweens.add({
      targets: txt,
      alpha: 0,
  ...
  private fxEliminated(seat: Anchor) {
    this.burst(seat, 0x202020, 30, 1.4);
  ...
  private burst(at: Anchor, color: number, count: number, scale = 1) {
    for (let i = 0; i < count; i++) {
      ...
      const dot = this.add.circle(at.x, at.y, 3 * scale, color, 1);
      this.tweens.add({
        targets: dot,
